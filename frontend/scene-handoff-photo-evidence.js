...
const SCENE_HANDOFF_VERSION = 'scene-handoff-0.5-v4.3-photo-evidence';
...
function decorateSceneHandoffButton() {
  ...
  helper.innerHTML = `Ensuite, envoyez à l’IA <strong>deux fichiers</strong> dans la même conversation neutre : ce handoff et le PDF photo original <strong>${PHOTO_EVIDENCE_FILENAME}</strong>. Le Survey reste autoritatif ; le PDF sert uniquement à reconstruire la géométrie.`;
...
document.addEventListener('click', async event => {
  ...
  if (status) status.textContent = 'Préparation du handoff Survey → Scene v4.3 avec preuves photo…';
  try {
