...
function makePartMesh(part) {
  ...
  mesh.userData = part;
  const edges = new THREE.LineSegments(new THREE.EdgesGeometry(geometry), new THREE.LineBasicMaterial({ color: 0x151922, transparent: true, opacity: 0.45 }));
  mesh.add(edges);
  return mesh;
}
...
function configureAssembly(bundle) {
  ...
  if (!plan || !plan.steps?.length) {
    ...
    for (const mesh of meshByPlacementId.values()) mesh.visible = true;
    return;
  }
...
function showAssemblyStep(index) {
  ...
  for (let i = 0; i <= clamped; i += 1) {
    for (const id of plan.steps[i].placement_ids) visibleIds.add(id);
  ...
  assemblyProgress.textContent = `Étape ${step.sequence}/${plan.total_steps} · ${cumulative}/${plan.total_parts} pièces`;
