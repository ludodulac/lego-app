...
def _canonical_ordering(fact: HumanRelativeLevelFact) -> tuple[str, str] | None:
    if fact.relation == "lower_than": return fact.subject_observation_id, fact.object_observation_id
    if fact.relation == "higher_than": return fact.object_observation_id, fact.subject_observation_id
    return None

def validate_human_relative_level_facts(survey: ArchitecturalSurvey, facts: list[HumanRelativeLevelFact]) -> HumanRelativeLevelFactSet:
    if not facts:
    ...
    for fact in facts:
        ...
        if ordered is None:
            if unordered in same_pairs: raise ValueError("duplicate human same-level fact")
            if _ordering_conflict(unordered, ordering_pairs): raise ValueError("human relative-level facts contradict same_level")
            same_pairs.add(unordered); continue
        if ordered in ordering_pairs: raise ValueError("duplicate human relative-level ordering fact")
        if (ordered[1], ordered[0]) in ordering_pairs: raise ValueError("human relative-level facts contain contradictory inverse ordering")
        if unordered in same_pairs: raise ValueError("human relative-level facts contradict same_level")
        ordering_pairs.add(ordered)
    return HumanRelativeLevelFactSet(source_survey_id=survey.id, facts=facts)

def _ordering_conflict(unordered: tuple[str, str], ordering_pairs: set[tuple[str, str]]) -> bool:
    a, b = unordered
