...
def _validate_refinement_semantics(survey: ArchitecturalSurvey) -> list[SurveyValidationIssue]:
    ...
    for observation in survey.observations:
        ...
        if not isinstance(target_id, str) or not target_id.strip():
            issues.append(SurveyValidationIssue("invalid_refinement_target", observation.id, "attributes.refines_observation_id doit contenir l’id non vide d’une observation antérieure.")); continue
        if target_id == observation.id:
            issues.append(SurveyValidationIssue("self_refinement", observation.id, "Une observation ne peut pas se raffiner elle-même.")); continue
        target = observations.get(target_id)
        if target is None:
            issues.append(SurveyValidationIssue("refinement_target_missing", observation.id, f"L’observation raffinée {target_id!r} n’existe pas dans ce Survey.")); continue
        if target.kind is not observation.kind:
            issues.append(SurveyValidationIssue("refinement_kind_changed", observation.id, f"Une observation de type {observation.kind.value!r} ne peut pas raffiner une observation de type {target.kind.value!r}."))
        if target.certainty is Certainty.CERTAIN or bool(target.attributes.get("confirmed_by_user", False)):
            issues.append(SurveyValidationIssue("certain_observation_cannot_be_refined", observation.id, f"L’observation {target_id!r} est déjà certaine ou confirmée par l’utilisateur. Une extension photo ne peut pas la réécrire ; utilisez le workflow de correction explicite en cas d’erreur."))
        ...
        if not any(item.photo_index not in previous_photos for item in observation.evidence):
            issues.append(SurveyValidationIssue("refinement_without_new_evidence", observation.id, f"Le raffinement de {target_id!r} doit citer au moins une preuve photo qui n’était pas déjà dans l’observation raffinée."))
    for observation in survey.observations:
        ...
        while True:
            ...
            if target_id in seen:
                issues.append(SurveyValidationIssue("refinement_cycle", observation.id, "La chaîne de raffinement contient une boucle ; elle doit rester chronologique et append-only.")); break
...
def validate_survey_semantics(survey: ArchitecturalSurvey) -> list[SurveyValidationIssue]:
    ...
    for observation in survey.observations:
        ...
        if observation.kind is ObservationKind.CONTEXT and target_ownership == "proven":
            issues.append(SurveyValidationIssue("target_building_element_downgraded_to_context", observation.id, "Un élément dont l’appartenance au bâtiment cible est prouvée ne peut pas être classé context. Utilisez son kind architectural natif ou signalez une limitation de contrat."))
        ...
        if expected_kind is not None and observation.kind is not expected_kind:
            issues.append(SurveyValidationIssue("architectural_kind_mismatch", observation.id, f"L’observation déclare attributes.architectural_kind={architectural_kind!r} mais kind={observation.kind.value!r}. Elle doit utiliser kind={expected_kind.value!r}."))
        if observation.kind is ObservationKind.OPENING:
            ...
            if confirmed and (not isinstance(semantic_role, str) or not semantic_role.strip()):
                issues.append(SurveyValidationIssue("confirmed_opening_missing_semantic_role", observation.id, "User-confirmed opening requires a stable semantic_role; visual ambiguity may not erase its identity."))
            if observation.certainty in {Certainty.CERTAIN, Certainty.PLAUSIBLE} and target_ownership == "unproven":
                issues.append(SurveyValidationIssue("opening_target_ownership_unproven", observation.id, "Opening ownership by the target building is unproven; mark the observation unproven or context until new evidence exists."))
            for field in ("facade_horizontal_rank", "facade_vertical_rank"):
                value = attributes.get(field)
                if value is not None and not _valid_positive_rank(value):
                    issues.append(SurveyValidationIssue("invalid_opening_layout_rank", observation.id, f"{field} doit être un entier positif 1..N lorsqu’il est renseigné. Ces rangs sont qualitatifs et ne doivent pas contenir de mètres, pixels ou texte libre."))
        if observation.kind is ObservationKind.ROOF:
            ...
            if observation.facade is not None and observation.facade.value == "front":
                ...
                if gable_end and roof_edge == "eave_across_facade":
                    issues.append(SurveyValidationIssue("gable_eave_terminology_conflict", observation.id, "A gable-end front facade cannot simultaneously have a horizontal eave across that facade; use rake/gable-edge terminology."))
            # A roof seen in several exterior views must not silently lose every
            # qualitative shape hypothesis. The Survey remains non-metric: this
            # requires only a cautious categorical hypothesis, never pitch/height.
            if len({item.photo_index for item in observation.evidence}) >= 2 and not any(
                name in attributes for name in ("roof_type", "facade_is_gable", "facade_roof_relationship", "roof_edge_type")
            ):
                issues.append(SurveyValidationIssue(
                    code="multiview_roof_missing_shape_hypothesis",
                    observation_id=observation.id,
                    message="Une toiture recoupée dans plusieurs vues ne peut pas perdre toute hypothèse qualitative de forme. Conservez roof_type/facade_is_gable ou une relation de rive avec sa certitude d’attribut ; n’inventez aucune métrique.",
                ))
...
def validate_survey_extension(base: ArchitecturalSurvey, candidate: ArchitecturalSurvey) -> list[SurveyValidationIssue]:
    ...
    candidate_photos = {photo.photo_index: photo for photo in candidate.photos}; base_photo_indexes = {photo.photo_index for photo in base.photos}
