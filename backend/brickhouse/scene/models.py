...
from brickhouse.building import (
    Appearance, Facade, OpeningType, Position3D, RidgeDirection, RoofType,
    SourceInfo, WindowStyle,
)
...
class SceneOpening(BaseModel):
    ...
    def validate_window_metadata(self) -> "SceneOpening":
        if self.type is not OpeningType.WINDOW:
            if any(v is not None for v in (self.window_style, self.has_sill, self.has_decorative_surround)):
...
class PlatformEdges(BaseModel):
    """Treat each physical platform edge independently; never infer symmetry."""
    x_min: EdgeTreatment = EdgeTreatment.UNKNOWN
    x_max: EdgeTreatment = EdgeTreatment.UNKNOWN
    y_min: EdgeTreatment = EdgeTreatment.UNKNOWN
    y_max: EdgeTreatment = EdgeTreatment.UNKNOWN
...
class Platform(BaseModel):
    ...
    # Legacy whole-perimeter hint. New scenes should use edges.
...
class VisibilityState(str, Enum):
    VISIBLE="visible"; OCCLUDED="occluded"; UNKNOWN="unknown"
...
class ArchitecturalScene(BaseModel):
    ...
    def validate_scene(self):
        self._validate_ids_and_references(); self._validate_opening_geometry(); self._validate_visibility(); self._validate_external_connectivity(); return self
    ...
    @staticmethod
    def _openings_overlap(a,b):
        return a.offset_horizontal < b.offset_horizontal+b.width-EPSILON and b.offset_horizontal < a.offset_horizontal+a.width-EPSILON and a.offset_vertical < b.offset_vertical+b.height-EPSILON and b.offset_vertical < a.offset_vertical+a.height-EPSILON
    ...
    def _validate_visibility(self):
        ...
        for e in self.visibility:
            if len(self.volumes)==1:
                ...
                for s in e.spans:
                    if s.to_offset>span+EPSILON: raise ValueError(f"visibility span on {e.facade.value} extends past facade")
    ...
    @staticmethod
    def _point_on_platform(point,p):
        return p.position.x-CONNECTIVITY_TOLERANCE_M<=point.x<=p.position.x+p.width+CONNECTIVITY_TOLERANCE_M and p.position.y-CONNECTIVITY_TOLERANCE_M<=point.y<=p.position.y+p.depth+CONNECTIVITY_TOLERANCE_M and abs(point.z-p.position.z)<=CONNECTIVITY_TOLERANCE_M
    @staticmethod
    def _point_on_volume_boundary(point,v):
        ...
        inside=x0-CONNECTIVITY_TOLERANCE_M<=point.x<=x1+CONNECTIVITY_TOLERANCE_M and y0-CONNECTIVITY_TOLERANCE_M<=point.y<=y1+CONNECTIVITY_TOLERANCE_M
        edge=min(abs(point.x-x0),abs(point.x-x1),abs(point.y-y0),abs(point.y-y1))<=CONNECTIVITY_TOLERANCE_M
        return inside and edge and z0-CONNECTIVITY_TOLERANCE_M<=point.z<=z1+CONNECTIVITY_TOLERANCE_M
    @staticmethod
    ...
    def _validate_external_connectivity(self):
        ...
        for s in self.stairs:
            for name,pt in (("start",s.start),("end",s.end)):
                connects=any(self._point_on_platform(pt,p) for p in self.platforms) or any(self._point_on_volume_boundary(pt,v) for v in self.volumes) or pt.z<=CONNECTIVITY_TOLERANCE_M
                if not connects: raise ValueError(f"stair {s.id!r} {name} does not connect to ground, a platform, or the building")
