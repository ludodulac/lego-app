...
EPSILON=1e-9;CONNECTIVITY_TOLERANCE_M=.12
class Evidence(BaseModel):photo_index:int=Field(ge=1);observation:str=Field(min_length=1)
class PropertyValue(BaseModel):value:float=Field(gt=0);source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list)
class SceneVolume(BaseModel):id:str;position:Position3D;width:PropertyValue;depth:PropertyValue;height:PropertyValue;floors:int=Field(ge=1,le=10);source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list)
class SceneOpening(BaseModel):
    id:str;type:OpeningType;volume_id:str;facade:Facade;offset_horizontal:float=Field(ge=0);offset_vertical:float=Field(ge=0);width:float=Field(gt=0);height:float=Field(gt=0);source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list);local_grade_clearance:float|None=None;window_style:WindowStyle|None=None;has_sill:bool|None=None;has_decorative_surround:bool|None=None
    @model_validator(mode="after")
    def validate_window_metadata(self):
        if self.type is not OpeningType.WINDOW and any(v is not None for v in (self.window_style,self.has_sill,self.has_decorative_surround)):raise ValueError("window metadata may only be set for window openings")
        return self
class SceneRoof(BaseModel):
    id:str;volume_id:str;type:RoofType;overhang:float=Field(ge=0);ridge_direction:RidgeDirection|None=None;pitch_degrees:float|None=None;source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list)
    @model_validator(mode="after")
...
class GradeProfile(BaseModel):
    facade:Facade;start_elevation:float;end_elevation:float;outward_extent:float|None=Field(default=None,gt=0);source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list)
class Terrain(BaseModel):kind:Literal["facade_grade_profiles"]="facade_grade_profiles";profiles:list[GradeProfile]=Field(default_factory=list)
class Chimney(BaseModel):id:str;position:Position3D;width:float=Field(gt=0);depth:float=Field(gt=0);height:float=Field(gt=0);source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list)
class ExteriorMaterial(str,Enum):TIMBER="timber";CONCRETE="concrete";MASONRY="masonry";STONE="stone";METAL="metal";COMPOSITE="composite";UNKNOWN="unknown"
class DeckBoardDirection(str,Enum):X="x";Y="y";UNKNOWN="unknown"
class EdgeTreatment(str,Enum):NONE="none";OPEN_RAILING="open_railing";SOLID_PARAPET="solid_parapet";WALL_ATTACHED="wall_attached";ACCESS_OPENING="access_opening";UNKNOWN="unknown"
class EdgeAccessSpan(BaseModel):
    from_offset:float=Field(ge=0,alias="from");to_offset:float=Field(gt=0,alias="to");model_config={"populate_by_name":True}
    @model_validator(mode="after")
...
class PlatformEdge(BaseModel):
    treatment:EdgeTreatment=EdgeTreatment.UNKNOWN;access_spans:list[EdgeAccessSpan]=Field(default_factory=list)
    @model_validator(mode="after")
    def validate_access(self):
        ...
        return self
class PlatformEdges(BaseModel):x_min:PlatformEdge=Field(default_factory=PlatformEdge);x_max:PlatformEdge=Field(default_factory=PlatformEdge);y_min:PlatformEdge=Field(default_factory=PlatformEdge);y_max:PlatformEdge=Field(default_factory=PlatformEdge)
class SupportPost(BaseModel):id:str;position:Position3D;width:float=Field(gt=0);depth:float=Field(gt=0);height:float=Field(gt=0);source:SourceInfo
class Platform(BaseModel):
    id:str;position:Position3D;width:float=Field(gt=0);depth:float=Field(gt=0);thickness:float=Field(gt=0);supports:list[SupportPost]=Field(default_factory=list);material:ExteriorMaterial|None=None;deck_board_direction:DeckBoardDirection|None=None;edge_treatment:EdgeTreatment|None=None;edges:PlatformEdges|None=None;source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list)
    @model_validator(mode="after")
    def validate_edge_spans(self):
        if self.edges is None:return self
        for name in ("x_min","x_max"):
            if any(s.to_offset>self.depth+EPSILON for s in getattr(self.edges,name).access_spans):raise ValueError(f"platform {self.id!r} access span on {name} exceeds depth")
        for name in ("y_min","y_max"):
            if any(s.to_offset>self.width+EPSILON for s in getattr(self.edges,name).access_spans):raise ValueError(f"platform {self.id!r} access span on {name} exceeds width")
        return self
class StairRun(BaseModel):id:str;start:Position3D;end:Position3D;width:float=Field(gt=0);material:ExteriorMaterial|None=None;left_edge:EdgeTreatment|None=None;right_edge:EdgeTreatment|None=None;source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list)
class EquipmentType(str,Enum):UTILITY_BOX="utility_box";PIPE="pipe";GUTTER="gutter";DOWNSPOUT="downspout";VENT="vent";ANTENNA="antenna";TEMPORARY_OBJECT="temporary_object"
class FacadeEquipment(BaseModel):id:str;type:EquipmentType;facade:Facade|None=None;source:SourceInfo;evidence:list[Evidence]=Field(default_factory=list)
class VisibilityState(str,Enum):VISIBLE="visible";OCCLUDED="occluded";UNKNOWN="unknown"
class VisibilitySpan(BaseModel):
    from_offset:float=Field(ge=0,alias="from");to_offset:float=Field(gt=0,alias="to");state:VisibilityState;by:str|None=None;model_config={"populate_by_name":True}
    @model_validator(mode="after")
    def validate_order(self):
...
class ArchitecturalScene(BaseModel):
    ...
    def _platform_touches_volume(p,v):
        px0,px1=p.position.x,p.position.x+p.width;py0,py1=p.position.y,p.position.y+p.depth;vx0,vx1=v.position.x,v.position.x+v.width.value;vy0,vy1=v.position.y,v.position.y+v.depth.value;xo=min(px1,vx1)>=max(px0,vx0)-CONNECTIVITY_TOLERANCE_M;yo=min(py1,vy1)>=max(py0,vy0)-CONNECTIVITY_TOLERANCE_M
