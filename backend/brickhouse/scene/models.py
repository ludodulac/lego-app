"""Metric ArchitecturalScene contracts between visual survey and LEGO construction."""
...
EPSILON=1e-6
CONNECTIVITY_TOLERANCE_M=.15

class SourceKind(str,Enum): OBSERVED="observed"; USER_PROVIDED="user_provided"; INFERRED="inferred"; GENERATED_DEFAULT="generated_default"
class SourceInfo(BaseModel): kind:SourceKind; confidence:float=Field(ge=0,le=1)
class Position3D(BaseModel): x:float; y:float; z:float
class Evidence(BaseModel): photo_index:int=Field(ge=1); observation:str
class PropertyValue(BaseModel): value:float=Field(gt=0); source:SourceInfo
class AppearanceColor(BaseModel): color:str
class Appearance(BaseModel): walls:AppearanceColor; roof:AppearanceColor; frames:AppearanceColor

class SceneVolume(BaseModel):
    id:str; position:Position3D; width:PropertyValue; depth:PropertyValue; height:PropertyValue; floors:int=Field(ge=1); source:SourceInfo; evidence:list[Evidence]=Field(default_factory=list)
class SceneOpening(BaseModel):
    id:str; volume_id:str; facade:Facade; type:OpeningType; offset_horizontal:float=Field(ge=0); offset_vertical:float=Field(ge=0); width:float=Field(gt=0); height:float=Field(gt=0); source:SourceInfo; evidence:list[Evidence]=Field(default_factory=list)
class SceneRoof(BaseModel):
    id:str; volume_id:str; type:RoofType; ridge_direction:RidgeDirection|None=None; source:SourceInfo; evidence:list[Evidence]=Field(default_factory=list)
class Chimney(BaseModel):
    id:str; position:Position3D; width:float=Field(gt=0); depth:float=Field(gt=0); height:float=Field(gt=0); source:SourceInfo; evidence:list[Evidence]=Field(default_factory=list)
class GradeProfile(BaseModel):
    facade:Facade; start_elevation:float; end_elevation:float; source:SourceInfo; evidence:list[Evidence]=Field(default_factory=list)
class Terrain(BaseModel): profiles:list[GradeProfile]=Field(default_factory=list)
...
class PlatformEdges(BaseModel):
    x_min:PlatformEdge=Field(default_factory=PlatformEdge); x_max:PlatformEdge=Field(default_factory=PlatformEdge); y_min:PlatformEdge=Field(default_factory=PlatformEdge); y_max:PlatformEdge=Field(default_factory=PlatformEdge)
class SupportPost(BaseModel):
    id:str; position:Position3D; width:float=Field(gt=0); depth:float=Field(gt=0); height:float=Field(gt=0); source:SourceInfo
class Platform(BaseModel):
    id:str; position:Position3D; width:float=Field(gt=0); depth:float=Field(gt=0); thickness:float=Field(gt=0)
    supports:list[SupportPost]=Field(default_factory=list); material:ExteriorMaterial|None=None; deck_board_direction:DeckBoardDirection|None=None
    edge_treatment:EdgeTreatment|None=None; edges:PlatformEdges|None=None; source:SourceInfo; evidence:list[Evidence]=Field(default_factory=list)
    @model_validator(mode="after")
...
class StairRun(BaseModel):
    id:str; start:Position3D; end:Position3D; width:float=Field(gt=0); material:ExteriorMaterial|None=None; left_edge:EdgeTreatment|None=None; right_edge:EdgeTreatment|None=None; source:SourceInfo; evidence:list[Evidence]=Field(default_factory=list)
