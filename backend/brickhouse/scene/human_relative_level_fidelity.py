...
def _scene_level(scene: ArchitecturalScene, observation_id: str) -> float | None:
    """Return a directly modeled walkable level only when semantics are unambiguous."""
    platform = next((item for item in scene.platforms if item.id == observation_id), None)
    if platform is not None:
        return platform.position.z
    return None


def validate_scene_against_human_relative_level_facts(
    ...
    for fact in facts:
        ...
        if subject_level is None or object_level is None:
            continue

        contradiction = False
        if fact.relation == "lower_than":
            contradiction = not subject_level < object_level - _LEVEL_EPSILON_M
        elif fact.relation == "higher_than":
            contradiction = not subject_level > object_level + _LEVEL_EPSILON_M
        elif fact.relation == "same_level":
            contradiction = abs(subject_level - object_level) > _LEVEL_EPSILON_M

        if contradiction:
            issues.append(
                HumanRelativeLevelIssue(
                    code="human_relative_level_contradicted",
                    subject_id=fact.subject_observation_id,
                    object_id=fact.object_observation_id,
                    message=(
                        f"Scene platform levels contradict user-provided relation {fact.subject_observation_id!r} "
                        f"{fact.relation} {fact.object_observation_id!r}; no default vertical separation may be invented."
                    ),
                )
            )
