...
def _host_is_secondary(observation)->bool: return bool(observation.attributes.get("host_object"))
...
def _stair_touches_platform(stair,platform)->bool:
    return any(_point_on_platform((p.x,p.y,p.z),platform) for p in (stair.start,stair.end))


def _stair_touches_stair(a,b)->bool:
...
def _platform_touches_platform(a,b)->bool:
...
def _edge_allows_access(edge,offset:float)->bool:
    ...
    return any(span.from_offset-CONNECTIVITY_TOLERANCE_M<=offset<=span.to_offset+CONNECTIVITY_TOLERANCE_M for span in edge.access_spans)


def _stair_platform_access_holds(stair,platform)->bool:
    """If the stair reaches a protected platform edge, require a declared gap there."""
    ...
    for p in (stair.start,stair.end):
        point=(p.x,p.y,p.z)
        if not _point_on_platform(point,platform):continue
        edges=[]
        if abs(p.x-x0)<=CONNECTIVITY_TOLERANCE_M: edges.append((platform.edges.x_min,p.y-y0))
        if abs(p.x-x1)<=CONNECTIVITY_TOLERANCE_M: edges.append((platform.edges.x_max,p.y-y0))
        if abs(p.y-y0)<=CONNECTIVITY_TOLERANCE_M: edges.append((platform.edges.y_min,p.x-x0))
        if abs(p.y-y1)<=CONNECTIVITY_TOLERANCE_M: edges.append((platform.edges.y_max,p.x-x0))
        # Endpoint already lands inside the deck: no perimeter guard blocks it.
        if not edges:return True
        candidates.extend(_edge_allows_access(edge,offset) for edge,offset in edges)
...
def validate_scene_against_survey(survey:ArchitecturalSurvey,scene:ArchitecturalScene)->list[SceneSurveyIssue]:
    ...
    if front_width is not None:
        ...
        if main is None or abs(main.width.value-front_width.value)>1e-6:
            issues.append(SceneSurveyIssue(code="front_width_measurement_drift",severity=SceneSurveySeverity.ERROR,object_id=main.id if main else None,message=f"La largeur avant mesurée dans le Survey vaut {front_width.value:g} m, mais la scène utilise {main.width.value if main else None!r}."))
    ...
    for o in scene.openings:
        ...
        for span in entry.spans:
            if span.state.value!="visible" and o.offset_horizontal<span.to_offset and o.offset_horizontal+o.width>span.from_offset:
                issues.append(SceneSurveyIssue(code="opening_in_hidden_span",severity=SceneSurveySeverity.ERROR,object_id=o.id,message=f"L’ouverture {o.id!r} intersecte une zone {span.state.value}.")); break
    ...
    ids_by_kind={ObservationKind.VOLUME:{i.id for i in scene.volumes},ObservationKind.PLATFORM:{i.id for i in scene.platforms},ObservationKind.STAIR:{i.id for i in scene.stairs}}
    codes={ObservationKind.VOLUME:"certain_volume_missing",ObservationKind.PLATFORM:"certain_platform_missing",ObservationKind.STAIR:"certain_stair_missing"}
    ...
    for relation in survey.relations:
        ...
        if stair is not None and not _stair_platform_access_holds(stair,platform):
            issues.append(SceneSurveyIssue(code="certain_connection_blocked_by_platform_edge",severity=SceneSurveySeverity.ERROR,object_id=stair.id,message=f"La relation certaine {relation.id!r} rejoint une plateforme derrière un garde-corps/muret continu. Déclare un access_span réel au point d’arrivée ou corrige la géométrie."))
    return issues
