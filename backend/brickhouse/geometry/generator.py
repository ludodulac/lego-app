...
def _opening_corners(volume: Volume, opening) -> list[Point3D]:
    ...
    if opening.facade is Facade.FRONT:
        return [
            _point(x0 + h0, y0, z_low),
            _point(x0 + h1, y0, z_low),
            _point(x0 + h1, y0, z_high),
            _point(x0 + h0, y0, z_high),
        ]
    if opening.facade is Facade.REAR:
        return [
            _point(x0 + volume.width - h0, y0 + volume.depth, z_low),
            _point(x0 + volume.width - h1, y0 + volume.depth, z_low),
            _point(x0 + volume.width - h1, y0 + volume.depth, z_high),
            _point(x0 + volume.width - h0, y0 + volume.depth, z_high),
        ]
    if opening.facade is Facade.LEFT:
        return [
            _point(x0, y0 + volume.depth - h0, z_low),
            _point(x0, y0 + volume.depth - h1, z_low),
            _point(x0, y0 + volume.depth - h1, z_high),
            _point(x0, y0 + volume.depth - h0, z_high),
        ]
    return [
        _point(x0 + volume.width, y0 + h0, z_low),
        _point(x0 + volume.width, y0 + h1, z_low),
        _point(x0 + volume.width, y0 + h1, z_high),
        _point(x0 + volume.width, y0 + h0, z_high),
    ]
    ...
    if roof.ridge_direction is RidgeDirection.DEPTH:
        ...
        return [
            RoofPlaneGeometry(
                ...
                corners=[
                    _point(x0, y0, eave_z),
                    _point(ridge_x, y0, ridge_z),
                    _point(ridge_x, y1, ridge_z),
                    _point(x0, y1, eave_z),
                ],
            ),
            RoofPlaneGeometry(
                id=f"{roof.id}:positive",
                roof_id=roof.id,
                volume_id=volume.id,
                roof_type=roof.type,
                side="positive",
    ...
    return [
        RoofPlaneGeometry(
            ...
            corners=[
                _point(x0, y0, eave_z),
                _point(x1, y0, eave_z),
                _point(x1, ridge_y, ridge_z),
                _point(x0, ridge_y, ridge_z),
            ],
        ),
        RoofPlaneGeometry(
            id=f"{roof.id}:positive",
        ...
        for facade in (Facade.FRONT, Facade.REAR, Facade.LEFT, Facade.RIGHT):
            opening_geometries = [
                OpeningGeometry(
                    ...
                    volume_id=volume.id,
                    facade=facade,
                    corners=_opening_corners(volume, opening),
                )
                for opening in openings_by_wall.get((volume.id, facade), [])
            ]
            walls.append(
                WallGeometry(
                    id=f"{volume.id}:{facade.value}",
                    volume_id=volume.id,
                    facade=facade,
                    corners=_wall_corners(volume, facade),
                    openings=opening_geometries,
                )
            )
