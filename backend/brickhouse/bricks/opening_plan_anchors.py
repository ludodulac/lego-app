...
def apply_opening_representation_plan(
    ...
    for wall in shell.walls:
        ...
        exact_compact_footprint = all(
            reservation.motif_id == "compact_two_pane:transparent_panels"
            and reservation.width_studs == raster.width_studs
            and reservation.height_bricks == raster.height_bricks
            for raster in wall.grid.openings
            if (reservation := wall_reservations.get(raster.id)) is not None
        )
        if exact_compact_footprint:
            facade_anchors = []
            for raster in wall.grid.openings:
