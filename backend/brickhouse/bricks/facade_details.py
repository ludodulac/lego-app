...
def generate_window_surrounds(building: BuildingModel, shell: BuildingBrickShell, *, skip_opening_ids: set[str] | None = None)->list[FacadeDetailPlacement]:
    """Create style-aware masonry fallback details inside window openings.

    Openings already filled by a validated real window assembly can be skipped;
    every other opening keeps the constructible brick surround used previously.
    """
