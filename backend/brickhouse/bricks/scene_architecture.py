...
def _volume_bounds(scene: ArchitecturalScene) -> tuple[float, float, float]:
    return (
        min(v.position.x for v in scene.volumes),
        min(v.position.y for v in scene.volumes),
        min(v.position.z for v in scene.volumes),
    )
...
def _stair_edge_treatments(
    stair: StairRun, scene: ArchitecturalScene
) -> tuple[EdgeTreatment, EdgeTreatment]:
    if stair.left_edge is not None or stair.right_edge is not None:
        return (
            stair.left_edge or EdgeTreatment.UNKNOWN,
            stair.right_edge or EdgeTreatment.UNKNOWN,
        )
    ...
    if legacy:
        return EdgeTreatment.SOLID_PARAPET, EdgeTreatment.SOLID_PARAPET
    return EdgeTreatment.UNKNOWN, EdgeTreatment.UNKNOWN
...
def _validate_exterior_primitives(scene: ArchitecturalScene) -> None:
    ...
    for platform in scene.platforms:
        ...
        if len(sides) > 1:
            raise ValueError(
                f"platform {platform.id!r} wraps a building corner; split it into one "
                "Platform per rectilinear facade segment"
            )
        side = sides[0]
        if side in {Facade.LEFT, Facade.RIGHT} and (
            ...
            raise ValueError(
                f"platform {platform.id!r} extends past a side-facade corner; split the geometry"
            )
        if side in {Facade.FRONT, Facade.REAR} and (
            x0 < left - EPSILON or x1 > right + EPSILON
        ):
            raise ValueError(
                f"platform {platform.id!r} extends past a front/rear corner; split the geometry"
            )
    for stair in scene.stairs:
        if abs(stair.end.x - stair.start.x) > EPSILON and abs(
            stair.end.y - stair.start.y
        ) > EPSILON:
            raise ValueError(
                f"stair {stair.id!r} changes two horizontal axes in one run; split "
                "turning stairs into axis-aligned StairRun objects joined by a landing"
            )
...
def _append_unique(
    parts: list[BrickModelPart], seen: set[tuple[int, int, int]], part: BrickModelPart
) -> bool:
...
def _platform_edge_cells(
    name: str, x0: int, y0: int, width: int, depth: int
) -> list[tuple[int, int]]:
    if name == "x_min":
        return [(x0, y0 + i) for i in range(depth)]
    if name == "x_max":
        return [(x0 + width - 1, y0 + i) for i in range(depth)]
    if name == "y_min":
        return [(x0 + i, y0) for i in range(width)]
    ...
    if treatment is EdgeTreatment.SOLID_PARAPET:
        for _, (x, y) in guarded:
            for z in range(z0 + 3, z0 + RAILING_HEIGHT_PLATES + 1, 3):
                ...
                if _append_unique(parts, seen, part):
                    index += 1
...
def _timber_deck(
    ...
    if direction is DeckBoardDirection.X:
        for dy in range(depth):
            ...
            while cursor < width:
                span, part_id = next(
                    (n, pid) for n, pid in catalog if n <= width - cursor
                )
                part = _brick(
        ...
        for dx in range(width):
            ...
            while cursor < depth:
                span, part_id = next(
                    (n, pid) for n, pid in catalog if n <= depth - cursor
                )
                part = _brick(
                    f"scene-platform:{platform.id}:board:{index:05d}",
                    x0 + dx,
...
def _masonry_platform_course_bases(
    platform: Platform, z0: int, plates_per_meter: float
) -> list[int]:
    """Return brick-course bases with the walking course anchored at platform.z.

    Stair endpoints and platform positions share the same Scene z-level. A thick
    concrete/masonry platform therefore has to grow downward from that level;
    growing upward would make a stair that is metrically connected arrive one or
    more LEGO courses below the visible landing surface.
    """
...
def _add_supports(
    ...
    *,
    ...
    """Render only explicit supports, respecting their observed footprint/height."""
    for support_index, support in enumerate(platform.supports, start=1):
        ...
        width = max(1, _round_half_up(support.width * studs_per_meter))
        depth = max(1, _round_half_up(support.depth * studs_per_meter))
        base_z = max(0, _round_half_up((support.position.z - origin_z) * plates_per_meter))
        declared_top = max(
            base_z,
            _round_half_up(
                (support.position.z + support.height - origin_z) * plates_per_meter
            ),
        )
            ...
            if treatment is EdgeTreatment.SOLID_PARAPET:
                for wall_z in (z + 3, z + 6):
                    if _append_unique(
                        ...
                        index += 1
            elif treatment is EdgeTreatment.OPEN_RAILING and (
                step in {0, steps} or step % RAILING_POST_SPACING_STUDS == 0
            ):
...
def _terrain_parts(
    ...
    *,
    ...
    index = 1
    for profile in scene.terrain.profiles:
        band = max(1, _round_half_up(_terrain_extent(profile) * studs_per_meter))
        length = width if profile.facade in {Facade.FRONT, Facade.REAR} else depth
        start = max(
            0,
            _round_half_up((profile.start_elevation - origin_z) * plates_per_meter),
        )
        end = max(
            0,
            _round_half_up((profile.end_elevation - origin_z) * plates_per_meter),
        )
        for along in range(length):
            t = along / max(length - 1, 1)
            grade = max(0, 3 * _round_half_up((start + (end - start) * t) / 3.0))
...
def augment_brick_model_with_scene_architecture(
    model: BrickModel, scene: ArchitecturalScene, *, front_width_studs: int
) -> BrickModel:
