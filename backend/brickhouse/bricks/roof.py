"""Support-aware gable roof using piece families present in the project catalog."""
...
def create_m0_roof_catalog()->RoofPartCatalog:
    """Roof subset mapped to IDs already present in piece_types_master.csv."""
    return RoofPartCatalog(parts=[
        RoofPartDefinition(id="BRICK_SLOPED_45_2X1",category="roof_tile",width_studs=2,length_studs=1,height_plates=3,connection_overlap_studs=1),
        RoofPartDefinition(id="BRICK_SLOPED_45_2X2",category="roof_tile",width_studs=2,length_studs=2,height_plates=3,connection_overlap_studs=1),
        RoofPartDefinition(id="BRICK_SLOPED_45_2X3",category="roof_tile",width_studs=2,length_studs=3,height_plates=3,connection_overlap_studs=1),
        RoofPartDefinition(id="BRICK_SLOPED_45_2X4",category="roof_tile",width_studs=2,length_studs=4,height_plates=3,connection_overlap_studs=1),
        RoofPartDefinition(id="TILE_1X1",category="ridge_tile",width_studs=1,length_studs=1,height_plates=1,connection_overlap_studs=1),
        RoofPartDefinition(id="TILE_1X2",category="ridge_tile",width_studs=1,length_studs=2,height_plates=1,connection_overlap_studs=1),
        RoofPartDefinition(id="TILE_1X4",category="ridge_tile",width_studs=1,length_studs=4,height_plates=1,connection_overlap_studs=1),
    ])
...
def _round(v:float)->int:return floor(v+.5)
def _tile_line(length:int,kind:str)->list[tuple[str,int,int]]:
    choices=((4,"BRICK_SLOPED_45_2X4"),(3,"BRICK_SLOPED_45_2X3"),(2,"BRICK_SLOPED_45_2X2"),(1,"BRICK_SLOPED_45_2X1")) if kind=="slope" else ((4,"TILE_1X4"),(2,"TILE_1X2"),(1,"TILE_1X1"))
    out=[]; cursor=0
...
def _plane_run_and_rise(plane:RoofPlaneGeometry)->tuple[float,float]:
    lo=min(p.z for p in plane.corners);hi=max(p.z for p in plane.corners);rise=hi-lo;e=[p for p in plane.corners if abs(p.z-lo)<1e-9];r=[p for p in plane.corners if abs(p.z-hi)<1e-9]
...
def validate_roof_support(roof:SpatialRoof,shell:BuildingBrickShell)->None:
    catalog=create_m0_roof_catalog();top=shell.walls[0].grid.height_bricks*3;width=next(r.grid.width_studs for r in shell.walls if r.facade.value=="front");depth=next(r.grid.width_studs for r in shell.walls if r.facade.value=="right")
    for side in ("negative","positive"):
        ...
        for prev,cur in zip(cs,cs[1:]):
            ...
            pz=min(p.z_plates for p in prev);cz=min(p.z_plates for p in cur)
            if cz<pz or cz-pz>6:raise ValueError(f"unsupported vertical jump on roof side {side!r}")
            if any(catalog.get(p.part_id).connection_overlap_studs<1 for p in cur):raise ValueError("roof part has no connection overlap")
...
def generate_spatial_gable_roof(g:BuildingGeometry,shell:BuildingBrickShell)->SpatialRoof:
    negative,_=_gable_planes(g,shell.volume_id);d=negative.ridge_direction;assert d is not None;run,rise=_plane_run_and_rise(negative);rise_per=(rise/run)*2.5;top=shell.walls[0].grid.height_bricks*3
    width=next(r.grid.width_studs for r in shell.walls if r.facade.value=="front");depth=next(r.grid.width_studs for r in shell.walls if r.facade.value=="right");span,line=(width,depth) if d is RidgeDirection.DEPTH else (depth,width);half=span//2;placements=[];counts={"negative":half,"positive":span-half-1}
    for side in ("negative","positive"):
        for distance in range(counts[side]):
            axis=distance if side=="negative" else span-2-distance;z=top+_round(distance*rise_per)
    ...
    ridge_axis=half;ridge_z=top+_round(max(0,half-1)*rise_per)+1
