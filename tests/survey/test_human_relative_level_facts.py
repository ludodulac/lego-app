...
def _survey() -> ArchitecturalSurvey:
    return ArchitecturalSurvey.model_validate(
        {
            "schema_version": "0.1",
            "id": "generic-level-survey",
            "name": "Generic level survey",
            "photos": [
                {
                    "photo_index": 1,
                    "facade": "front",
            ...
            "observations": [
                {
                    ...
                    "kind": "platform",
                    "certainty": "certain",
                    "statement": "First platform.",
                    "evidence": [{"photo_index": 1, "observation": "First platform visible."}],
                },
                {
                    "id": "landing-b",
                    "kind": "platform",
                    "certainty": "certain",
                    "statement": "Second platform.",
                    "evidence": [{"photo_index": 1, "observation": "Second platform visible."}],
                },
            ],
        }
    )
    ...
    return HumanRelativeLevelFact.model_validate(
        {
            ...
            "object_observation_id": object,
            "relation": relation,
            "certainty": "certain",
            "source": USER,
            "statement": "User confirms qualitative relative level only.",
        }
    )
...
def test_relative_level_fact_requires_user_provenance_and_existing_ids() -> None:
    ...
    payload["source"] = {"kind": "inferred", "confidence": 0.8}
...
def test_self_duplicate_inverse_and_same_level_conflicts_are_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="contradictory inverse"):
        validate_human_relative_level_facts(
            _survey(),
            [_fact(), _fact(subject="landing-b", object="deck-a", relation="lower_than")],
        )

    with pytest.raises(ValueError, match="contradict same_level"):
        validate_human_relative_level_facts(
            _survey(),
            [_fact(), _fact(relation="same_level")],
        )
