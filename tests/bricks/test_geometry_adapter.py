...
def _part(placement_id: str, part_id: str = "BRICK_1X1", *, x=0, y=0, z=0, turns=0):
    return BrickModelPart(
        placement_id=placement_id, part_id=part_id, category="brick", component="wall",
        x_studs=x, y_studs=y, z_plates=z, rotation_quarter_turns=turns, facade=Facade.FRONT,
    )


def _roof_part(placement_id: str, *, side: str, x=0, y=0, z=0, turns=0,
               part_id="BRICK_SLOPED_45_2X4"):
    return BrickModelPart(
        placement_id=placement_id, part_id=part_id, category="roof_tile", component="roof",
        x_studs=x, y_studs=y, z_plates=z, rotation_quarter_turns=turns, roof_side=side,
    )


def _box_definition(part_id: str, minimum, maximum, description="") -> PartDefinition:
    x0, y0, z0 = minimum; x1, y1, z1 = maximum
    p000=(x0,y0,z0); p001=(x0,y0,z1); p010=(x0,y1,z0); p011=(x0,y1,z1)
    p100=(x1,y0,z0); p101=(x1,y0,z1); p110=(x1,y1,z0); p111=(x1,y1,z1)
    triangles=(
        (p000,p100,p101),(p000,p101,p001),(p010,p011,p111),(p010,p111,p110),
        (p000,p010,p110),(p000,p110,p100),(p001,p101,p111),(p001,p111,p011),
        (p000,p001,p011),(p000,p011,p010),(p100,p110,p111),(p100,p111,p101),
    )
    return PartDefinition(part_id, triangles, AABB(minimum, maximum), description)


def _cube_definition():
    return _box_definition("3005.dat", (-10.0,0.0,-10.0), (10.0,24.0,10.0), "Brick 1 x 1")


def _slope_definition(ldraw_id: str, width: int, length: int):
    # Verified LDraw slope families use local X for longitudinal span and a
    # Z range ending at +10 LDU with the origin offset toward the high edge.
    return _box_definition(
        f"{ldraw_id}.dat",
        (-length * 10.0, 0.0, -(width * 20.0 - 10.0)),
        (length * 10.0, 24.0, 10.0),
        f"Slope {width} x {length}",
    )


class FakeLibrary:
    def __init__(self): self.loaded=[]; self.brick_definition=_cube_definition()
    def load_part(self, part_id: str):
        self.loaded.append(part_id)
        for mapping in CANONICAL_LDRAW_PARTS.values():
            if mapping.ldraw_id == part_id and mapping.placement_kind == "slope":
                return _slope_definition(part_id, mapping.width_studs, mapping.length_studs)
        return self.brick_definition


def test_mapping_covers_standard_bricks_and_all_modeled_roof_slope_ids():
    assert {brick.id for brick in create_m0_brick_catalog().bricks}.issubset(CANONICAL_LDRAW_PARTS)
    expected_roof = {
        "BRICK_SLOPED_18_4X2":"30363",
        "BRICK_SLOPED_33_3X6":"3939", "BRICK_SLOPED_33_3X4":"3297", "BRICK_SLOPED_33_3X2":"3298",
        "BRICK_SLOPED_45_2X4":"3037", "BRICK_SLOPED_45_2X3":"3038",
        "BRICK_SLOPED_45_2X2":"3039", "BRICK_SLOPED_45_2X1":"3040b",
    }
    assert {key: CANONICAL_LDRAW_PARTS[key].ldraw_id for key in expected_roof} == expected_roof
...
def test_adapter_preserves_placement_id_and_uses_verified_ldraw_id():
    library=FakeLibrary(); instance=brick_model_part_to_instance(_part("wall-000001"),library)
    assert library.loaded == ["3005"] and instance.instance_id == "wall-000001"


def test_two_canonical_bricks_on_consecutive_courses_contact_without_collision():
    library=FakeLibrary(); lower=brick_model_part_to_instance(_part("lower",z=0),library); upper=brick_model_part_to_instance(_part("upper",z=3),library)
    assert check_collision(lower,upper) is Relation.CONTACT


@pytest.mark.parametrize("part_id", [
    "BRICK_SLOPED_18_4X2", "BRICK_SLOPED_33_3X6", "BRICK_SLOPED_33_3X4",
    "BRICK_SLOPED_33_3X2", "BRICK_SLOPED_45_2X4", "BRICK_SLOPED_45_2X3",
    "BRICK_SLOPED_45_2X2", "BRICK_SLOPED_45_2X1",
])
def test_verified_slope_families_align_decentered_ldraw_bbox_to_grid(part_id):
    library=FakeLibrary(); mapping=CANONICAL_LDRAW_PARTS[part_id]
    instance=brick_model_part_to_instance(_roof_part("roof",side="negative",x=2,y=5,z=12,turns=0,part_id=part_id),library)
    assert instance.bbox.minimum[0] == pytest.approx(40.0)
    assert instance.bbox.maximum[0] == pytest.approx((2+mapping.width_studs)*20.0)
    assert instance.bbox.minimum[2] == pytest.approx(100.0)
    assert instance.bbox.maximum[2] == pytest.approx((5+mapping.length_studs)*20.0)
...
def test_positive_slope_reverses_rise_without_moving_footprint():
    library=FakeLibrary(); instance=brick_model_part_to_instance(_roof_part("roof-pos",side="positive",x=7,y=1,z=9),library)
...
def test_rotated_slope_aligns_when_ridge_runs_along_width_axis():
    library=FakeLibrary(); instance=brick_model_part_to_instance(_roof_part("roof-rot",side="negative",x=3,y=4,z=6,turns=1),library)
