...
def _roof(building_id="house"):
    return SpatialRoof(building_id=building_id,roof_id="roof",ridge_direction=RidgeDirection.DEPTH,placements=[GlobalRoofPlacement(part_id="BRICK_SLOPED_45_2X4",side="negative",x_studs=0,y_studs=0,z_plates=6,rotation_quarter_turns=0),GlobalRoofPlacement(part_id="BRICK_SLOPED_45_2X4",side="positive",x_studs=8,y_studs=0,z_plates=6,rotation_quarter_turns=0),GlobalRoofPlacement(part_id="TILE_1X2",side="ridge",x_studs=5,y_studs=0,z_plates=9,rotation_quarter_turns=0)])
...
def test_bom_aggregates_canonical_parts_and_totals():
    b=generate_bom(generate_brick_model(_shell(),_roof()));q={l.part_id:l.quantity for l in b.lines};assert q["BRICK_1X4"]==2;assert q["BRICK_1X2"]==1;assert q["BRICK_SLOPED_45_2X4"]==2;assert q["TILE_1X2"]==1;assert b.total_parts==6;assert b.unique_part_types==4
