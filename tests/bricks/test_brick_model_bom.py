...
def _roof(building_id="house"):
    return SpatialRoof(building_id=building_id,roof_id="roof",ridge_direction=RidgeDirection.DEPTH,placements=[GlobalRoofPlacement(part_id="ROOF_SLOPE_2X4",side="negative",x_studs=0,y_studs=0,z_plates=6,rotation_quarter_turns=0),GlobalRoofPlacement(part_id="ROOF_SLOPE_2X4",side="positive",x_studs=8,y_studs=0,z_plates=6,rotation_quarter_turns=0),GlobalRoofPlacement(part_id="RIDGE_TILE_1X2",side="ridge",x_studs=5,y_studs=0,z_plates=9,rotation_quarter_turns=0)])


def test_brick_model_merges_wall_and_roof_parts():
    model=generate_brick_model(_shell(),_roof())
    assert len(model.parts)==6
    assert sum(p.component=="wall" for p in model.parts)==3
...
def test_brick_model_generates_stable_unique_ids_and_metadata():
    model=generate_brick_model(_shell(),_roof())
    assert [p.placement_id for p in model.parts]==["wall-000001","wall-000002","wall-000003","roof-000001","roof-000002","roof-000003"]
...
def test_bom_aggregates_canonical_parts_and_totals():
    ...
    assert quantities["ROOF_SLOPE_2X4"]==2
    assert quantities["RIDGE_TILE_1X2"]==1
    assert bom.total_parts==6
    assert bom.unique_part_types==4
    assert sum(line.quantity for line in bom.lines)==bom.total_parts


def test_bom_order_and_serialization_are_deterministic():
    first=generate_bom(generate_brick_model(_shell(),_roof())); second=generate_bom(generate_brick_model(_shell(),_roof()))
    assert first.model_dump(mode="json")==second.model_dump(mode="json")
    assert [(l.category,l.part_id) for l in first.lines]==sorted((l.category,l.part_id) for l in first.lines)
