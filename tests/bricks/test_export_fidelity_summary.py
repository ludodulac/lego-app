...
def test_new_export_summarizes_empty_fidelity_issue_list() -> None:
    ...
    assert bundle.metadata.fidelity_summary == BrickExportFidelitySummary(
        info_count=0,
        warning_count=0,
        blocker_count=0,
        has_blockers=False,
...
def test_summary_counts_deduplicated_issues_not_raw_inputs() -> None:
    ...
    assert bundle.metadata.fidelity_summary.warning_count == 1
...
def test_summary_includes_automatically_generated_semantic_color_issue() -> None:
    ...
    assert bundle.metadata.fidelity_summary.info_count == 1
    assert bundle.metadata.fidelity_summary.warning_count == 0
    assert bundle.metadata.fidelity_summary.blocker_count == 0
    assert bundle.metadata.fidelity_summary.has_blockers is False
...
def test_historical_schema_v01_bundle_without_summary_still_parses() -> None:
    ...
    payload["metadata"].pop("fidelity_summary")

    parsed = BrickExportBundle.model_validate(payload)

    assert parsed.schema_version == "0.1"
    assert parsed.metadata.fidelity_summary is None
...
def test_present_summary_must_match_serialized_issue_severities() -> None:
    ...
    payload = current.model_dump(mode="json")
    payload["metadata"]["fidelity_summary"] = {
        "info_count": 0,
        "warning_count": 0,
        "blocker_count": 0,
        "has_blockers": False,
