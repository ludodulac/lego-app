...
def _model() -> BrickModel:
    return BrickModel(
        building_id="b1", volume_id="v1", width_studs=8, depth_studs=6, height_plates=10,
        parts=[
            ...
            BrickModelPart(placement_id="roof-000001", part_id="ROOF_SLOPE_2X2", category="roof_tile", component="roof", x_studs=0, y_studs=0, z_plates=6, rotation_quarter_turns=0, roof_side="negative"),
...
def test_plan_covers_every_part_exactly_once():
    model = _model(); plan = generate_assembly_plan(model)
...
def test_walls_are_before_roof_and_levels_are_bottom_up():
    ...
    assert [(step.component, step.z_plates) for step in plan.steps] == [("wall",0),("wall",3),("roof",6),("roof",9)]


def test_step_ids_sequences_and_part_order_are_deterministic():
    first=generate_assembly_plan(_model()); second=generate_assembly_plan(_model())
    assert first==second
