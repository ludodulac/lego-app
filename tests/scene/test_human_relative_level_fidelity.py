...
def _survey() -> ArchitecturalSurvey:
    return ArchitecturalSurvey.model_validate(
        {
            "schema_version": "0.1",
            "id": "generic-level-fidelity-survey",
            "name": "Generic level fidelity survey",
...
def _scene(deck_z: float, landing_z: float) -> ArchitecturalScene:
    return ArchitecturalScene.model_validate(
        {
            "schema_version": "0.2",
            "id": "generic-level-fidelity-scene",
            "name": "Generic level fidelity scene",
            "units": "m",
            "volumes": [{
                "id": "host",
                "position": {"x": 0, "y": 0, "z": 0},
                "width": {"value": 6, "source": SOURCE},
...
def _fact() -> HumanRelativeLevelFact:
    return HumanRelativeLevelFact.model_validate(
        {
            "subject_observation_id": "deck-a",
            "object_observation_id": "landing-b",
            "relation": "lower_than",
            "certainty": "certain",
            "source": USER,
            "statement": "Deck is lower than landing; no metric delta supplied.",
        }
    )


def test_any_positive_separation_satisfies_non_metric_lower_than_fact() -> None:
    report = validate_scene_against_human_relative_level_facts(_survey(), _scene(2.0, 2.01), [_fact()])
