...
def test_active_manual_handoff_uses_two_stage_survey_then_scene_flow() -> None:
    ...
    assert "UNIQUEMENT l’objet ArchitecturalSurvey v0.1" in survey_generator
...
def test_manual_scene_fixture_validates_and_cross_checks_against_validated_survey() -> None:
    ...
    cross = client.post(
        ...
        json={"survey": survey_raw, "scene": scene_raw},
    )
...
def test_topology_summary_cannot_masquerade_as_architectural_survey() -> None:
    topology_like = {
        ...
        "topology": {
            "facades": ["front"],
            "objects": [{"kind": "opening", "statement": "One opening is visible."}],
        },
        "summary": "This is reasoning output, not the complete Survey contract.",
    }
