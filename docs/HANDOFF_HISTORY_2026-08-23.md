...
> Archive historique de l’ancien `HANDOFF.md`. Conservée pour ne pas perdre les décisions et le contexte de continuité antérieurs. Pour reprendre le projet aujourd’hui, lire `AI_START_HERE.md`, `NEXT_CONVERSATION.md` et `docs/CURRENT_PROJECT_STATE.md`.
...
Cette section décrivait le point de reprise au 23/08/2026 et est désormais historique.

Test utilisateur de l’époque : les 5 photos originales + un ZIP `brickhouse-photos-a-analyser.zip` avaient été envoyés dans une conversation IA vierge, mais l’IA n’exécutait pas spontanément l’instruction enfermée dans le ZIP.

Les évolutions ultérieures ont remplacé ce point de reprise par le workflow manuel en deux étapes Photos → Survey puis Survey + PDF → Scene. Voir `docs/CURRENT_PROJECT_STATE.md`.

## 19. Ancien prochain test utilisateur

Le protocole de test décrit ici a été supersédé. Ne pas le suivre comme instruction actuelle. Consulter `NEXT_CONVERSATION.md` et `docs/CURRENT_PROJECT_STATE.md`.

## 20. Commits structurants historiques

Liste indicative conservée pour contexte :
- `aeb50628` généralisation du prompt au-delà de la maison test ;
- `c6d2e2c3`, `d0521f8b`, `8fb5f959` suppression de biais maison test ;
...
- Ne solliciter l’utilisateur que lorsqu’une action humaine est réellement nécessaire.

## 22. Ancienne priorité de reprise

La priorité décrite ici au 23/08/2026 est supersédée. L’état courant est dans `docs/CURRENT_PROJECT_STATE.md`.
